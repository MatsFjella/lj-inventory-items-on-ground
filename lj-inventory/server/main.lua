	...
	if not ply.state.inv_busy then
		if name and id then
			...
			elseif name == "trunk" then
				...
				secondInv.maxweight = other.maxweight or 600000
				secondInv.inventory = {}
...
RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
	...
	if fromInventory == "player" or fromInventory == "hotbar" then
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "traphouse" then
				...
				-- drop putting stuff on the ground
	...
	elseif fromInventory == "attachment_crafting" then
		...
		-- drop picking stuff from the ground
	...
	if Player then
			if itemData then
				...
				if AddItem(id, itemData["name"], amount, false, info) then
					...
					QBCore.Functions.Notify(source, "Kan ikke gi!", "error")
				end
			else
				QBCore.Functions.Notify(source, "Ingen gjenstand funnet", "error")
			end
	else
		QBCore.Functions.Notify(source,  "Spiller er ikke online", "error")
	end
end, "admin")
...
CreateUsableItem("id_card", function(source, item)
	...
	for _, v in pairs(players) do
		...
		if dist < 3.0 then
			...
			TriggerClientEvent('chat:addMessage', v,  {
					template = '<div class="chat-message advert"><div class="chat-message-body"><strong>{0}:</strong><br><br> <strong>Civ ID:</strong> {1} <br><strong>Fornavn:</strong> {2} <br><strong>Etternavn:</strong> {3} <br><strong>Fødselsdato:</strong> {4} <br><strong>Kjønn:</strong> {5} <br><strong>Nasjonalitet:</strong> {6}</div></div>',
					args = {
						"ID Card",
						item.info.citizenid,
						item.info.firstname,
